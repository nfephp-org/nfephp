...
set_time_limit(1800);
require(dirname(__file__).DIRECTORY_SEPARATOR.'MPDF56'.DIRECTORY_SEPARATOR.'mpdf.php');
require(dirname(__file__).DIRECTORY_SEPARATOR.'qrcode'.DIRECTORY_SEPARATOR.'qrcode.class.php');
...
class DanfeNFCeNFePHP extends CommonNFePHP implements DocumentoNFePHP {
    ...
    public function monta($orientacao='',$papel=array(80,'one-page'),$logoAlign='C',$situacao_externa=NFEPHP_SITUACAO_EXTERNA_NONE,$CLASSE_PDF=false){
    ...
    public function montaDANFE($orientacao='',$papel=array(80,'one-page'),$logoAlign='C',$situacao_externa=NFEPHP_SITUACAO_EXTERNA_NONE,$CLASSE_PDF=false){
...
#			"<td style='border-bottom:1px dotted black'><img src=\"$IMG_CHAVE\"></td>".
	...
	foreach ($this->det as $d) {
		...
		else			$desconto=number_format($desconto, 2, ",", ".");
...
#echo 5;
	...
	$endereco_consumidor = $lgr . ", " . $nro . $cpl . "\n" . $bairro . " - " . $CEP . "\n" . $mun . " - " . $UF . " " . "Fone/Fax: " . $fone;
	$HTML.=	($doc!=''?
			"<table width='100%'>\n".
			...
			"<tr><td style='border-bottom:1px dotted black;width:100%'>".htmlspecialchars($endereco_consumidor)."</td></tr>\n".
			"</table>\n":'').
		"</body></html>\n";
