 ...
 * sob os termos da LicenÃ§a PÃºblica Geral GNU (GPL)como Ã© publicada pela FundaÃ§Ã£o
 * para o Software Livre, na versÃ£o 3 da licenÃ§a, ou qualquer versÃ£o posterior
 * e/ou
 * sob os termos da LicenÃ§a PÃºblica Geral Menor GNU (LGPL) como Ã© publicada pela FundaÃ§Ã£o
 * para o Software Livre, na versÃ£o 3 da licenÃ§a, ou qualquer versÃ£o posterior.
 *
 *
 ...
 * Caso contrÃ¡rio consulte <http://www.fsfla.org/svnwiki/trad/GPLv3> ou
 * <http://www.fsfla.org/svnwiki/trad/LGPLv3>.
 *
 * EstÃ¡ atualizada para :
 *      PHP 5.3
 *      VersÃ£o 3.10 dos webservices da SEFAZ com comunicaÃ§Ã£o via SOAP 1.2
 *      e conforme Manual de IntegraÃ§Ã£o VersÃ£o 5
 *
 * AtenÃ§Ã£o: Esta classe nÃ£o mantÃªm a compatibilidade com a versÃ£o 2.00 da SEFAZ !!!
 *
 * @package   NFePHP
 * @name      ToolsNFePHP
 ...
 *              Eduardo GusmÃ£o <eduardo.intrasis at gmail dot com>
 ...
 *              Jorge Luiz Rodrigues TomÃ© <jlrodriguestome at hotmail dot com>
...
class ToolsNFePHP
{
    ...
    /**
     * Sefaz Virtual de ContingÃªncia Rio Grande do Sul (SVC-RS)
     */
    ...
    /**
     * Sefaz Virtual de ContingÃªncia Ambiente Nacional (SVC-AN)
     */
    const CONTINGENCIA_SVCAN = 'SVCAN';

    /**
     * Sistema de ContingÃªncia do Ambiente Nacional  (SC-AN)
     */
     ...
     * "XX" Ã© o modelo da NF-e, "55" ou "65")
     */
    public $xmlURLfile = 'nfe_ws3_mod55.xml';
    /**
     * enableSVAN
     * Indica o acesso ao serviÃ§o SVAN: Sefaz Virtual Ambiente Nacional
     * @var boolean
     */
    public $enableSVAN = false;
    /**
     * enableSVRS
     * Indica o acesso ao serviÃ§o SVRS: Sefaz Virtual Rio Grande do Sul
     * @var boolean
     */
    ...
    /**
     * Modelo da NF-e: 55 (NF-e) ou 65 (NFC-e), default 55 pois Ã© a mais comum
     * atualmente
     * @var integer 
     */
    public $modelo = 55;
    ...
    /**
     * aliaslist
     * Lista dos aliases para os estados que usam SEFAZ prÃ³pria ou virtual
     * @var array
     */
     ...
     *
     * @param array   $aConfig   Opcional dados de configuraÃ§Ã£o
     * @param integer $mododebug Opcional 2-NÃ£o altera nenhum parÃ¢metro 1-SIM ou 0-NÃƒO (2 default)
     * @param boolean $exceptions Opcional Utilize true para gerar exceÃ§Ãµes
     ...
     * @param    array   $aError VariÃ¡vel passada como referencia irÃ¡ conter as mensagens de erro se houverem
     * @return   boolean
     */
    public function validXML($xml = '', $xsdFile = '', &$aError = array())
    {
        try {
            ...
            if (strlen($xml)==0) {
                $msg = 'VocÃª deve passar o conteudo do xml assinado como parÃ¢metro '
                       .'ou o caminho completo atÃ© o arquivo.';
                $aError[] = $msg;
                throw new nfephpException($msg);
            }
            ...
            if (!empty($errors)) {
                ...
                $msg = 'O dado informado nÃ£o Ã© um XML ou nÃ£o foi encontrado. '
                        . 'VocÃª deve passar o conteudo de um arquivo xml assinado como parÃ¢metro.';
                $aError[] = $msg;
                throw new nfephpException($msg);
            }
            if ($xsdFile=='') {
    ...
    public function addProt($nfefile = '', $protfile = '')
    {
        try {
            ...
            if (!isset($protNFe) && !isset($retCancNFe) && !isset($retEvento)) {
                $msg = 'O arquivo indicado para ser protocolado a NFe nÃ£o Ã© um protocolo nem de cancelamento! '
                       .$protfile;
                throw new nfephpException($msg);
            }
            if ($chNFe != $chave) {
     ...
     * @param string $b2bfile path para o arquivo xml padrÃ£o ANFAVEA+GS1 e NT2013_002
    ...
    public function addB2B($nfefile = '', $b2bfile = '', $tagB2B = '')
    {
        try {
            ...
            if (! $b2b->loadXML($xmlb2b, LIBXML_NOBLANKS | LIBXML_NOEMPTYTAG)) {
                $msg = 'O arquivo indicado como Protocolo nÃ£o Ã© um XML! '.$b2bfile;
                throw new nfephpException($msg);
            }
     ...
     * @name statusServico
     * @param  string $siglaUF sigla da unidade da FederaÃ§Ã£o
     * @param  integer $tpAmb tipo de ambiente 1-produÃ§Ã£o e 2-homologaÃ§Ã£o
    ...
    public function statusServico($siglaUF = '', $tpAmb = '', &$aRetorno = array())
    {
        try {
            ...
            //verifica se contingencias SVCAN ou SVCRS estÃ£o habilitadas, se nÃ£o estiverem
            //usarÃ¡ a prÃ³pria UF, logo abaixo ao carregar os webservices
            if ($this->enableSVCAN) {
            ...
            if ($cStat == '') {
                throw new nfephpException("NÃ£o houve retorno Soap verifique a mensagem de erro e o debug!!");
            } elseif ($cStat == '107') { //107-serviÃ§o em operaÃ§Ã£o
                $aRetorno['bStat'] = true;
            }
            // tipo de ambiente
     ...
     * retornados podem ser bastante incompletos. NÃ£o Ã© recomendado seu uso.
     *
     * @name consultaCadastro
     * @param  string  $siglaUF sigla da unidade da federaÃ§Ã£o
     * @param   string  $IE opcional numero da inscriÃ§Ã£o estadual
     * @param   string  $CNPJ opcional numero do cnpj
     * @param   string  $CPF opcional numero do cpf
     * @param   string  $tpAmb tipo de ambiente se nÃ£o informado serÃ¡ usado o ambiente default
    ...
    public function consultaCadastro($siglaUF, $CNPJ = '', $IE = '', $CPF = '', $tpAmb = '')
    {
        ...
        $namespace = $this->URLPortal.'/wsdl/'.$servico.'2';
        if ($urlservico=='') {
            $msg = "Este serviÃ§o nÃ£o estÃ¡ disponÃ­vel para a SEFAZ $siglaUF!!!";
            $this->pSetError($msg);
            if ($this->exceptions) {
                throw new nfephpException($msg);
            }
     ...
     * ATENÃ‡ÃƒO! Este Ã© o antigo mÃ©todo "sendLot()" que enviava lotes de NF-e versÃ£o "2.00"
     * consumindo o WS "NfeRecepcao2", agora este mÃ©todo estÃ¡ preparado apenas para a versÃ£o
     * "3.10" e por isso utiliza o WS "NfeAutorizacao" sempre em modo sÃ­ncrono.
     *
     * @name autoriza
     * @package NFePHP
     * @author Roberto L. Machado <linux.rlm at gmail dot com>
     * @param string  $sxml   string com uma nota fiscal em xml
     * @param integer $idLote id do lote e um numero (numeraÃ§Ã£o sequencial)
     * @param array   $aRetorno parametro passado por referencia contendo a resposta da consulta em um array
     * @param integer $indSinc IndicaÃ§Ã£o webservice assÃ­ncrono (0) ou sÃ­ncrono (1)
     * @return mixed string XML do retorno do webservice, ou false se ocorreu algum erro 
     */
    public function autoriza($sxml, $idLote, &$aRetorno = array(), $indSinc = 1)
    {
        try {
            //retorno do mÃ©todo em array (esta estrutura espelha a estrutura do XML retornado pelo webservice
            ...
            if ($indSinc === 0) {
                ...
                $aRetorno['protNFe'] = array(
                    'versao'=>'',
                    'infProt'=>array( //informaÃ§Ãµes do protocolo de autorizaÃ§Ã£o da NF-e
                        'tpAmb'=>'',
                        'verAplic'=>'',
                        'chNFe'=>'',
                        'dhRecbto'=>'',
            ...
            if (!@$this->enableSCAN) {   // TODO 08/Julho fmertins: esta prop nÃ£o existe mais, verificar como serÃ¡ refatorado (commit https://github.com/nfephp-org/nfephp/commit/bfbb78393e582b8a4291c441a31ef17c58b024fa)
                $aURL = $this->aURL;
            } else {
                $aURL = $this->pLoadSEFAZ($this->tpAmb, self::CONTINGENCIA_SCAN);
            }
            //identificaÃ§Ã£o do serviÃ§o: autorizaÃ§Ã£o de NF-e
            ...
            //valida o parÃ¢metro da string do XML da NF-e
            ...
            if ($cStat == '') {
                ...
                $aRetorno['bStat'] = true;
            } elseif ($indSinc === 1 && $cStat == '104') { //104-Lote processado, podendo ter ou nÃ£o o protNFe (#AR11 no layout)
                $aRetorno['bStat'] = true;
            } else {
            ...
            //tipo do ambiente, versÃ£o do aplicativo e cÃ³digo da UF
            ...
            if ($indSinc == 1) {
               //retorno sÃ­ncrono do webservice: dados do protocolo da NF-e
               ...
               //nÃºmero do protocolo de autorizaÃ§Ã£o (opcional)
               ...
               //retorno assÃ­ncrono do webservice: dados do recibo do lote
            ...
            //grava o retorno na pasta de temporÃ¡rios
            $nome = $doc->save($nome);
        } catch (nfephpException $e) {
            $this->pSetError($e->getMessage());
            if ($this->exceptions) {
     ...
     * @param   string   $tpAmb  numero do ambiente 1-producao e 2-homologaÃ§Ã£o
     ...
     *
     * @name getListNFe
     * @param boolean $ambNac TRUE - usa ambiente Nacional para buscar a lista de NFe, FALSE usa sua prÃ³pria SEFAZ
     * @param string $indNFe Indicador de NF-e consultada: 
     * 0=Todas as NF-e; 1=Somente as NF-e que ainda nÃ£o tiveram manifestaÃ§Ã£o do destinatÃ¡rio 
     * (Desconhecimento da operaÃ§Ã£o, OperaÃ§Ã£o nÃ£o Realizada ou ConfirmaÃ§Ã£o da OperaÃ§Ã£o); 
     * 2=Idem anterior, incluindo as NF-e que tambÃ©m nÃ£o tiveram a CiÃªncia da OperaÃ§Ã£o
     ...
     * que nÃ£o tenham a mesma raiz do CNPJ do destinatÃ¡rio (para excluir as notas fiscais 
     * de transferÃªncia entre filiais).
     * @param string $ultNSU Ãšltimo NSU recebido pela Empresa. 
     * Caso seja informado com zero, ou com um NSU muito antigo, a consulta 
     * retornarÃ¡ unicamente as notas fiscais que tenham sido recepcionadas 
     * nos Ãºltimos 15 dias.
     * @param string $tpAmb Tipo de ambiente 1=ProduÃ§Ã£o 2=HomologaÃ§Ã£o
     ...
     * para as NF-e confirmadas pelo destinatÃ¡rio. As NFe baixadas serÃ£o salvas
     ...
     * @param   string  $nSerie     serie da NF 1 atÃ© 3 digitos
     * @param   integer $nIni       numero inicial 1 atÃ© 9 digitos zero a esq
     * @param   integer $nFin       numero Final 1 atÃ© 9 digitos zero a esq
     * @param   string  $xJust      justificativa 15 atÃ© 255 digitos
     * @param   string  $tpAmb      Tipo de ambiente 1-produÃ§Ã£o ou 2 homologaÃ§Ã£o
    ...
    public function inutNF($nAno = '', $nSerie = '1', $nIni = '', $nFin = '', $xJust = '', $tpAmb = '', &$aRetorno = array())
    {
        ...
        if ($nAno == '' || $nIni == '' || $nFin == '' || $xJust == '') {
            $msg = "NÃ£o foi passado algum dos parametos necessÃ¡rios ANO=$nAno inicio=$nIni "
    ...
    public function cancelEvent($chNFe = '', $nProt = '', $xJust = '', $tpAmb = '', &$aRetorno = array())
    {
        try {
            ...
            if ($chNFe == '' || $nProt == '' || $xJust == '') {
                $msg = "NÃ£o foi passado algum dos parÃ¢metros necessÃ¡rios "
            ...
            if ($cStat == '') {
                //houve erro
                $msg = "cStat estÃ¡ em branco, houve erro na comunicaÃ§Ã£o Soap "
     ...
     * @name envCCe
     * @param   string $chNFe Chave da NFe
     * @param   string $xCorrecao DescriÃ§Ã£o da CorreÃ§Ã£o entre 15 e 1000 caracteres
     * @param   string $nSeqEvento numero sequencial da correÃ§Ã£o d 1 atÃ© 20
    ...
    public function envCCe($chNFe = '', $xCorrecao = '', $nSeqEvento = '1', $tpAmb = '', &$aResp = array())
    {
        ...
        try {
            ...
            if ($nSeqEvento == '' || strlen($nSeqEvento) > 2 || !is_numeric($nSeqEvento) || $nSeqEvento < 1) {
                $msg = "NÃºmero sequencial da correÃ§Ã£o nÃ£o encontrado ou Ã© maior "
                        . "que 99 ou contÃªm caracteres nÃ£o numÃ©ricos [$nSeqEvento]";
                throw new nfephpException($msg);
            }
     ...
     * @param   integer $tpAmb Tipo de ambiente
     * @param   mixed  $resp variÃ¡vel passada como referencia e irÃ¡ conter o retorno da funÃ§Ã£o em um array
     * @return mixed false
     *
        ...
        try {
            ...
            switch ($tpEvento) {
                case '210200':
                    ...
                    //confirma a operaÃ§Ã£o e o recebimento da mercadoria (para as operaÃ§Ãµes
                    //com circulaÃ§Ã£o de mercadoria)
                    //ApÃ³s a ConfirmaÃ§Ã£o da OperaÃ§Ã£o pelo destinatÃ¡rio, a empresa emitente
                ...
                case '210240':
                    ...
                    //nÃ£o aceitaÃ§Ã£o no recebimento que antes se fazia com apenas um carimbo na NF
                    ...
                    break;
                default:
                    $msg = "O cÃ³digo do tipo de evento informado nÃ£o corresponde a "
                        . "nenhum evento de manifestaÃ§Ã£o de destinatÃ¡rio.";
                    throw new nfephpException($msg);
            }
     ...
     * @param string $tag tag que Ã© assinada
     * @param string $err variavel passada como referencia onde sÃ£o retornados os erros
    ...
    public function verifyNFe($file)
    {
        try {
            ...
            if ($nProtSefaz != $nProt) {
                $msg = "Os numeros dos protocolos nÃ£o combinam!! nProtNF = "
                      .$nProt." <> nProtSefaz = ".$nProtSefaz."";
                throw new nfephpException($msg);
            }
            //verifica o digest
            if ($digestSefaz != $digest) {
                $msg = "Os numeros digest nÃ£o combinam!! digValSEFAZ = "
     ...
     * @param  string $sUF       Sigla da Unidade da FederaÃ§Ã£o (ex. SP, RS, etc..)
     ...
     *
     * @name loadCerts
     * @param  boolean $testaVal True testa a validade do certificado ou false nÃ£o testa
     ...
     * @param    array   $aRetorno variavel passa por referÃªncia Array com os dados do certificado
    ...
    /**
     * listDir
     * Obtem todo o conteÃºdo de um diretorio, e que atendam ao critÃ©rio indicado.
     * @param string $dir Diretorio a ser pesquisado
     * @param string $fileMatch CritÃ©rio de seleÃ§Ã£o pode ser usados coringas como *-nfe.xml
     ...
     * @param type $tpAmb tipo de ambiente 1-produÃ§Ã£o ou 2 homologaÃ§Ã£o
                    ...
                    foreach ($objxml->UF as $objElem) {
                        ...
                        if ($objElem->sigla == $siglaUF) {
                            ...
                            if (!file_put_contents($nfews, $objxml->asXML())) {
                                $msg = "A versÃ£o do serviÃ§o $servico de $siglaUF [$sAmbiente] "
                                       ."no arquivo $nfews nÃ£o foi corrigida.";
                                throw new nfephpException($msg, self::STOP_MESSAGE);
                            } else {
                                break;
                            }//fim file_put
    ...
    protected function pConvertTime($dataHora = '')
    {
        ...
        if ($dataHora) {
            ...
            $adDH = explode('-', $aDH[0]);
            $atDH = explode(':', substr($aDH[1], 0, 8));//substring para recuperar apenas a hora, sem o fuso horÃ¡rio
    ...
    {
        $aFind = array('&','Ã¡','Ã ','Ã£','Ã¢','Ã©','Ãª','Ã­','Ã³','Ã´','Ãµ','Ãº','Ã¼',
            'Ã§','Ã�','Ã€','Ãƒ','Ã‚','Ã‰','ÃŠ','Ã�','Ã“','Ã”','Ã•','Ãš','Ãœ','Ã‡');
     ...
     *
     * @name pSetError
     * @param   string $msg DescriÃ§Ã£o do erro
     * @return  none
     */
