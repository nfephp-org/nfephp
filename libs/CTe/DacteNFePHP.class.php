...
*/
	...
	protected function descricaoCargaDACTE($x = 0, $y = 0)
	{
		...
		$texto = number_format( ( empty($texto) ? 0 : $texto ) , 2 , "," , ".");
		$aFont = $this->formatNegrito;
		$this->__textBox($x + 1, $y + 3, $w, $h, $texto, $aFont, 'T', 'L', 0, '');
		$y += 8;
		$x = $oldX;
		...
		$texto = $this->__simpleGetValue($this->infQ->item(3), "qCarga") != "" ?
				$this->__simpleGetValue($this->infQ->item(3), "qCarga") :
				0;
	...
	protected function docOrigDACTE($x = 0, $y = 0)
	{
		...
		$texto = (($tpCTe == 1) ? 'DETALHAMENTO DO CT-E COMPLEMENTADO' : 'DOCUMENTOS ORIGINÁRIOS');
		$aFont = $this->formatPadrao;
		$this->__textBox($x, $y, $w, $h, $texto, $aFont, 'T', 'C', 1, '');
		if ($tpCTe == 1) { // Se for CT-e Complementar
			$valor = number_format($this->__simpleGetValue($this->vPrest, "vTPrest"), 2, ",", ".");
			...
			$texto = $descr1;
			$aFont = $this->formatPadrao;
			$this->__textBox($x, $y, $w * 0.30, $h, $texto, $aFont, 'T', 'L', 0, '');
			$yIniDados = $y;
			$x += $w * 0.35;
			$texto = $descr2;
			$aFont = $this->formatPadrao;
			...
			$texto = $this->__simpleGetValue($this->infCte, "chave");
			$aFont = array('font' => $this->fontePadrao, 'size' => 8, 'style' => '');
			...
			$this->__textBox($auxX, $yIniDados, $w * 0.14, $h, $texto, $aFont, 'T', 'L', 0, '');
			$auxX += $w * 0.23;
		} // FIM Se for CT-e Complementar
		else {
			$descr1 = 'TIPO DOC';
			$descr2 = 'CNPJ/CHAVE';
			$descr3 = 'S?IE/NRO. DOCUMENTO';
			$y += 3.4;
			$this->pdf->Line($x, $y, $w + 1, $y);
			$texto =  $descr1 ;
			$aFont = $this->formatPadrao;
	...
	protected function obsDACTE($x = 0, $y = 0)
	{
		...
		//Novos valores de impostos (2015-003-icms-interestadual)
		$vBCUFFim = $this->__simpleGetValue($this->imp, "vBCUFFim", "");
		$pICMSUFFim = $this->__simpleGetValue($this->imp, "pICMSUFFim", "");
		$pICMSInter = $this->__simpleGetValue($this->imp, "pICMSInter", "");
		$pICMSInterPart = $this->__simpleGetValue($this->imp, "pICMSInterPart", "");
		$vICMSUFFim = $this->__simpleGetValue($this->imp, "vICMSUFFim", "");
		$vICMSUFIni = $this->__simpleGetValue($this->imp, "vICMSUFIni", "");
		//Novos valores de impostos (2015-004-Fundo de Combate à pobreza)
		$pFCPUFFim = $this->__simpleGetValue($this->imp, "pFCPUFFim", "");
		$vFCPUFFim = $this->__simpleGetValue($this->imp, "vFCPUFFim", "");

		//Verifica se um dos valores existe e coloca os valores mesmo que 0 no campo de observação
		if ( ! empty( $vBCUFFim ) && ! empty( $pICMSUFFim ) && ! empty( $pICMSInter ) && ! empty( $pICMSInterPart ) && ! empty( $vICMSUFFim ) && ! empty( $vICMSUFIni ) && ! empty( $pFCPUFFim ) && ! empty( $vFCPUFFim ) ) {
			$texto_impostos = sprintf(
									 "Valor BC do ICMS %s ; Percentual de ICMS correspondente ao
Fundo de Combate à pobreza %s ; Alíquota interna da UF %s ; Alíquota interestadual das UF %s ; Percentual provisório de partilha entre os estados %s ; Valor do ICMS correspondente ao Fundo
de Combate à pobrez %s ; Valor do ICMS de partilha para a UF de témino %s ; Valor do ICMS de partilha para a UF de início %s" ,
									number_format( $vBCUFFim , 2 , "," , "." ) ,
									number_format( $pFCPUFFim , 2 , "," , "." ) ,
									number_format( $pICMSUFFim , 2 , "," , "." ) ,
									number_format( $pICMSInter , 2 , "," , "." ) ,
									number_format( $pICMSInterPart , 2 , "," , "." ) ,
									number_format( $vFCPUFFim , 2 , "," , "." ) ,
									number_format( $vICMSUFFim , 2 , "," , "." ) ,
									number_format( $vICMSUFIni , 2 , "," , "." )
									) ;

			$texto .= $texto_impostos ;
		}
