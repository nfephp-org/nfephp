    ...
    */
    ...
    public function montaDANFE(
        ...
        if (isset($this->retirada)) {
            ...
            $this->textoAdic .= "LOCAL DE RETIRADA : " .$txRetCNPJ. '-' . $txRetxLgr . ', '
        ...
        if (isset($this->infAdic)) {
            ...
            $this->textoAdic .= !empty($this->infAdic->getElementsByTagName("infCpl")->item(0)->nodeValue) ?
                'Inf. Contribuinte: '
                . trim($this->__anfavea($this->infAdic->getElementsByTagName("infCpl")->item(0)->nodeValue)) : '';
        ...
        $flagVTT = $flagVTT &&
                (strpos(strtolower(trim($this->textoAdic)), 'trib') ||
        ...
        $hDispo1 = $this->hPrint - ($hcabecalho + $hdestinatario +
                ($linhasDup * $hduplicatas) + $himposto + $htransporte +
                ($linhaISSQN * $hissqn) + $hdadosadic + $hfooter +
                $hCabecItens + $this->__sizeExtraTextoFatura());
        if ($this->orientacao == 'P') {
    ...
    {
        return $this->situacao_externa == NFEPHP_SITUACAO_EXTERNA_DPEC &&
                $this->numero_registro_dpec!='';
    }
    ...
    protected function __destinatarioDANFE($x = 0, $y = 0)
    {
        ...
        if (!empty($this->dest->getElementsByTagName("CNPJ")->item(0)->nodeValue)) {
            ...
            $texto = !empty($this->dest->getElementsByTagName("CPF")->item(0)->nodeValue) ?
                $this->__format($this->dest->getElementsByTagName("CPF")->item(0)->nodeValue, "###.###.###-##") : '';
        }
        $aFont = array('font'=>$this->fontePadrao, 'size'=>10, 'style'=>'B');
        $this->__textBox($x, $y, $w, $h, $texto, $aFont, 'B', 'C', 0, '');
        //DATA DA EMISSÃO
        ...
        $texto = !empty($this->dest->getElementsByTagName("fone")->item(0)->nodeValue) ?
                $this->__format($this->dest->getElementsByTagName("fone")->item(0)->nodeValue, '(##) ####-####') : '';
        $aFont = array('font'=>$this->fontePadrao, 'size'=>10, 'style'=>'B');
        $this->__textBox($x, $y, $w, $h, $texto, $aFont, 'B', 'C', 0, '');
        //INSCRIÇÃO ESTADUAL
        $x += $w;
        ...
        if ($this->dup->length > 0 || $textoFatura !== "") {
            ...
            foreach ($this->dup as $k => $d) {
                ...
                $dDup = !empty($this->dup->item($k)->getElementsByTagName('dVenc')->item(0)->nodeValue) ?
                        $this->__ymd2dmy($this->dup->item($k)->getElementsByTagName('dVenc')->item(0)->nodeValue) : '';
    ...
    protected function __impostoDANFE($x, $y)
    {
        ...
        if ($this->exibirPIS=='1') {
            ...
            $valorImposto = !empty($this->ICMSTot->getElementsByTagName("vPIS")->item(0)->nodeValue) ?
                number_format($this->ICMSTot->getElementsByTagName("vPIS")->item(0)->nodeValue, 2, ", ", ".") : '0, 00';
        } else {
            $texto = '';
            $valorImposto = '';
        }
        ...
        $valorImposto = !empty($this->ICMSTot->getElementsByTagName("vDesc")->item(0)->nodeValue) ?
                number_format(
                    $this->ICMSTot->getElementsByTagName("vDesc")->item(0)->nodeValue,
                    2,
                    ", ",
                    "."
                ) : '0, 00';
    ...
    protected function __descricaoProduto($itemProd)
    {
        ...
        $infAdProd = substr(
            !empty($itemProd->getElementsByTagName('infAdProd')->item(0)->nodeValue) ?
            $this->__anfavea($itemProd->getElementsByTagName('infAdProd')->item(0)->nodeValue) :
            '',
            0,
            500
        );
            ...
            if ($i >= $nInicio) {
                ...
                if (isset($ICMS)) {
                    $texto = !empty($ICMS->getElementsByTagName("pICMS")->item(0)->nodeValue) ?
                            number_format($ICMS->getElementsByTagName("pICMS")->item(0)->nodeValue, 2, ", ", ".") :
                        '0, 00';
    ...
    protected function __dadosAdicionaisDANFE($x, $y, $h)
    {
        ...
        //3 – Contingência SCAN – emissão em contingência no Sistema de Contingência
        //do Ambiente Nacional – SCAN;
        //4 – Contingência DPEC - emissão em contingência com envio da Declaração Prévia
        //de Emissão em Contingência – DPEC;
        //5 – Contingência FS-DA - emissão em contingência com impressão do DANFE
        //em Formulário de Segurança para Impressão de Documento Auxiliar de Documento Fiscal Eletrônico (FS-DA).
     ...
     * Exemplo: NFe Ref.: série: 01 número: 01 emit: 11.111.111/0001-01 em 
     * 10/2010 [0000 0000 0000 0000 0000 0000 0000 0000 0000 0000 0000]
     *
