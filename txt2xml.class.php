		...
		if ($handle) {
			...
			while (!feof($handle)) {
				...
				switch (strtoupper(trim($dados[0]))) {
					...
					case "X04":	
						if(!$this->vazio($dados[1])){
                                                //$DOCTRAN = $dom->createElement("CNPJ", trim($dados[1]));
							$CNPJ = $dom->createElement("CNPJ", trim($dados[1]));
							$transporta->appendChild($CNPJ);  //POG ATE RESLVOER
						}
                                                break;
					case "X05":	
						if(!$this->vazio($dados[1])){
                                                //$DOCTRAN = $dom->createElement("CPF", trim($dados[1]));
							$CPF = $dom->createElement("CPF", trim($dados[1]));
							$transporta->appendChild($CPF); //POG ATE RESOLVER
						}                                                		
						break;
                                        case "X03"://TODO VEREFICAR TAG CNPJ CPF ANTES DO NOME
                                        $transporta = $dom->createElement("transporta");
                                                          //$transporta->appendChild($DOCTRAN);
					...
					case "X33":
						...
						$lacre = $dom->createElement("lacre", trim($dados[1]));
						$lacres->appendChild($placa);
